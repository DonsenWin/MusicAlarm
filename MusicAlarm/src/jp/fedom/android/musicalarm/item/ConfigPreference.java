...
public class ConfigPreference {
	...
	/**
	 * Reset all config items.
	 */
	public final void resetAllConfigItems() {
	...
	public final List<ConfigItem> loadConfigItems() {
		...
		if (jsonData == null || jsonData.isEmpty()) {
			...
			try {
				items = (ArrayList<ConfigItem>) ConfigItem.JsonParser
						.getInstance().parse(jsonData);
	...
	public final void saveConfigItems(final List<ConfigItem> items) {
		if (items != null) {
			...
			try {
				editor.putString(PREFERENCE_KEY, ConfigItem.JsonGenerator
						.getInstance().genJson(items));
				editor.clear().apply();
			} catch (JSONException e) {
				// do nothing
